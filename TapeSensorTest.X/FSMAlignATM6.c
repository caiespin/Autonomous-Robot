...
#define FORWARD_TIME 75
#define STOP_MOTOR_TIME 200
#define TANK_TURN_TIME 675
...
static TemplateSubHSMState_t CurrentState = InitPState; // <- change name to match ENUM
...
uint8_t InitFSMAlignAtm6(void) {
    ES_Event returnEvent;

    CurrentState = InitPState;
    returnEvent = RunFSMAlignAtm6(INIT_EVENT);
...
ES_Event RunFSMAlignAtm6(ES_Event ThisEvent) {
    ...
    switch (CurrentState) {
        ...
        case Stop1State:
            switch (ThisEvent.EventType) {
                ...
                case ES_TIMEOUT:
                    if (ThisEvent.EventParam == ALIGN_ATM6_TIMER) {
    ...
    if (makeTransition == TRUE) { // making a state transition, send EXIT and ENTRY
        ...
        RunFSMAlignAtm6(EXIT_EVENT); // <- rename to your own Run function
        CurrentState = nextState;
        RunFSMAlignAtm6(ENTRY_EVENT); // <- rename to your own Run function
    }
