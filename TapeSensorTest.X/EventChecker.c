...
void Print_Event(ES_Event thisEvent) {
    printf("Event Generated:%d, Param:%d\r\n", thisEvent.EventType, thisEvent.EventParam);
}

static int light_on = FALSE;

int get_light_status(){
    return light_on;
}

uint8_t CheckLights(void) {
    //static ES_EventTyp_t lastEvent = ES_NO_EVENT;
    ES_EventTyp_t curEvent;
    ES_Event thisEvent;
    uint8_t returnVal = FALSE;
    uint16_t light_level = Roach_LightLevel(); // read the light level

    //static int light_on = FALSE;
    if (light_level < LIGHT_THRESHOLD&&light_on == FALSE) { // 
        light_on = TRUE;
        curEvent = INTO_LIGHT;
        thisEvent.EventType = curEvent;
        thisEvent.EventParam = light_level;
        returnVal = TRUE;
        Post_HSM_Service(thisEvent);
    } else if (light_level > DARK_THRESHOLD&&light_on == TRUE) {
        light_on = FALSE;
        curEvent = INTO_DARK;
        thisEvent.EventType = curEvent;
        thisEvent.EventParam = light_level;
